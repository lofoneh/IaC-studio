...
func (h *ProjectsHandler) Create(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeErrorStr(w, http.StatusBadRequest, "invalid json"); return }
    if err := validators.New().Struct(req); err != nil { writeErrorStr(w, http.StatusBadRequest, err.Error()); return }
    ...
    if err := h.repo.Create(r.Context(), &p); err != nil { writeError(w, http.StatusInternalServerError, err); return }
